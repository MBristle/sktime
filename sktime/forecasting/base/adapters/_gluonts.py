...
class GluonTS(BaseForecaster):
    ...
    def _fit(self, y, X, fh):
        ...
        # We have to bring the data into the required format for gluonts
        ...
        extended_y = self._generate_extended_y(df, self.model_parameter, self.fh_length)
        extended_y["item_id"] = "a"
        dataset = PandasDataset.from_long_dataframe(
            extended_y.reset_index(), target=extended_y.columns[0], item_id="item_id"
        )
        training_data, test_gen = split(dataset, offset=-self.fh_length)
        predict_input = test_gen.generate_instances(
            prediction_length=self.fh_length, windows=1
        ).input
        self._predict_input = predict_input
        if fh:
            self._fh = fh
        self._instantiate_model()
        if hasattr(self._forecaster, "train"):  # Predictors have no train method.
            self._forecaster = self._forecaster.train(training_data)
        return self
    ...
    def _predict(self, fh=None, X=None):
        ...
        if fh is not None and all(fh != self._fh):
            IndentationError(
                f"Training FH ({self._fh}) was not the same as prediction FH ({fh})!"
            )

        return list(self._forecaster.predict(self._predict_input))[0].mean_ts

    def _instantiate_model(self):
        """Instantiate the model based on the provided model parameters.

        This function checks if the 'prediction_length' parameter in the
        'model_parameter' dictionary is provided and is equal to the
        'fh_length' (ForcastingHorizon length). If not, it raises a ValueError.
        After that, it sets the 'prediction_length' in the 'model_parameter'
        dictionary to the 'fh_length'.
        Finally, it instantiates the '_forecaster' attribute of the class with
        the provided model parameters.
        ...
        if model_parameter is None:
            model_parameter = {}
        if (
            "prediction_length" in model_parameter.keys()
            and model_parameter["prediction_length"] != self.fh_length
        ):
            raise ValueError(
                "prediction_length in model_parameters must be "
                "the same as the ForcastingHorizon fh."
            )
        model_parameter["prediction_length"] = self.fh_length
        self._forecaster = self._ModelClass(**model_parameter)
        return self

    def _generate_extended_y(df, model_parameters, fh_length):
        """Generate extended y with future timestamps.

        This function generates an extended version of the DataFrame `df` by appending
        future timestamps based on the provided forecast horizon length
        `fh_length` and model parameters `model_parameters`.

        Parameters
        ----------
            df (pd.DataFrame): The input DataFrame.
            model_parameters (dict): A dictionary containing the model parameters.
            fh_length (int): The length of the forecast horizon.

        Returns
        -------
            pd.DataFrame: The extended DataFrame with future timestamps appended.
        """
        if not isinstance(df.index, pd.DatetimeIndex):
            idx_max = df.index[-1] + 1
            int_idx = pd.date_range(start="2000-01-01", periods=idx_max, freq="H")
            int_idx = int_idx[df.index]
            df.index = int_idx

        if "freq" not in model_parameters:
            freq = pd.infer_freq(df.index)
            if len(freq) > 1 and freq.endswith("S"):
                freq = freq[-1]
            model_parameters["freq"] = freq

        extended_y = df.reindex(
            df.index.append(
                pd.date_range(
                    start=df.index.values[-1],
                    periods=(fh_length + 1),
